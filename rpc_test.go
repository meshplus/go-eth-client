...
func TestEthRPC_EthCall(t *testing.T) {
	...
	require.Nil(t, err)
	args, err := Encode(&contractAbi, "store", "5")
	require.Nil(t, err)
...
func prepareContract(t *testing.T, cli Client, privateKey *ecdsa.PrivateKey, contractAbi abi.ABI, code string, abiEvent *utils.AbiEvent) string {
	...
	assert.Nil(t, err)
	input1, err := utils.Decode(abiEvent, "registerOrg", bodyBytes1)
	require.Nil(t, err)
	...
	assert.Nil(t, err)
	input2, err := utils.Decode(abiEvent, "registerUser", bodyBytes2)
	require.Nil(t, err)
...
func TestInvokeTupleContract(t *testing.T) {
	...
	assert.Nil(t, err)
	args, err := utils.Decode(abiEvent, "publish", bodyBytes)
	assert.Nil(t, err)
